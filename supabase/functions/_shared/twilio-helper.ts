// File 1: supabase/functions/_shared/twilio-helper.ts
// This file should contain the corrected VoiceResponse class and validation.
// Based on artifact: twilio_helper_class_fix_v1
...
class VoiceResponse {
  ...
  private _currentTagStack: string[]; 

  constructor() {
    this._content = ['<?xml version="1.0" encoding="UTF-8"?>'];
    this._currentTagStack = []; 
  ...
  say(attributesOrText: Record<string, any> | string, textToSay?: string): this {
    ...
    const finalMessage = (message && message.trim() !== "") ? message.trim() : " "; 
  ...
  redirect(attributes: { method?: string } = { method: 'POST' }, url: string): this {
    ...
    // The URL for Redirect content should NOT be XML encoded here. Twilio expects a plain URL.
  ...
  gather(attributes: Record<string, any>, nestedCallback: (gatherNode: VoiceResponse) => void): this {
    ...
    // The 'action' URL attribute should be pre-encoded using encodeXmlUrl by the caller
