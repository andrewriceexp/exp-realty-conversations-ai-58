...
import { encodeXmlUrl, createErrorResponse, createDebugTwiML } from "../_shared/twiml-helpers.ts"; // Assuming createDebugTwiML is also in twiml-helpers now

serve(async (req) => {
  const requestTimestamp = new Date().toISOString();
  const functionVersion = "USING_CORRECTED_HELPER_V1"; 
  ...
  try {
    ...
    const fullUrlForValidation = url.toString(); // Full URL including query for GET, or base for POST for validation
    ...
    if (req.method === 'POST' && requestBodyForValidation) { // requestBodyForValidation will be empty if .text() failed
        try {
    ...
    // Use the full URL Twilio called for validation (including query string for GET, base for POST is handled in helper)
    const validationUrl = url.origin + url.pathname + (req.method === 'GET' ? url.search : '');

    if (!isStatusCallback) {
      console.log(`[${requestTimestamp}] (${functionVersion}) Attempting to validate Twilio request. Validation URL: ${validationUrl}`);
      // Pass requestBodyForValidation to the helper, which should only use it if method is POST
      const isValidRequest = await validateTwilioRequest(req, validationUrl, userTwilioAuthToken, bypassValidation /*, requestBodyForValidation - Handled by helper internally now */);

      if (!isValidRequest) {
        console.error(`[${requestTimestamp}] (${functionVersion}) Twilio request validation FAILED. URL: ${validationUrl}. Returning 403 Forbidden.`);
    ...
    if (debugMode) {
      ...
      // Assuming createDebugTwiML uses the new VoiceResponse class correctly
      const debugMessage = `Debug. SID: ${callSid || 'N/A'}. User: ${userId || 'N/A'}. Voice: ${voiceIdFromUrl || 'Default'}`;
      response.say({voice: 'alice'}, debugMessage); // Use the new say method
      console.log(`[${requestTimestamp}] (${functionVersion}) Debug <Say>: ${debugMessage}`);
      response.pause({ length: 1 });
    }
    
    // Determine greeting, voice settings
    ...
    let finalAgentVoiceId = voiceIdFromUrl || null; // Prioritize voice_id from URL
    let initialSayVoice = 'alice'; // Default Twilio voice
    ...
    if (agentConfigId) {
        try {
            ...
            if (agentConfig) {
                ...
                greeting = (agentConfig.system_prompt && agentConfig.system_prompt.trim() !== "") 
                    ? agentConfig.system_prompt.split(/[.!?]/)[0].trim() + "." 
                    : defaultGreeting;
                if (greeting.length <= 10) greeting = defaultGreeting; // Ensure meaningful greeting

                if (agentConfig.voice_provider === 'elevenlabs' && agentConfig.voice_id) {
                    if (!finalAgentVoiceId) finalAgentVoiceId = agentConfig.voice_id; // Use config voice if no URL voice
                    useElevenLabs = true;
        ...
        greeting = defaultGreeting;
        if (voiceIdFromUrl) { // If voice_id from URL, assume ElevenLabs if API key exists
             console.log(`[${requestTimestamp}] (${functionVersion}) No agentConfigId, but voiceIdFromUrl (${voiceIdFromUrl}) present.`);
        }
    }
    
    const elevenLabsApiKey = Deno.env.get('ELEVENLABS_API_KEY');
    if (finalAgentVoiceId && elevenLabsApiKey) { // If there's any voice ID (from URL or config) and API key
        useElevenLabs = true;
        initialSayVoice = 'Polly.Amy-Neural'; // Use a better base for ElevenLabs simulation
        ...
        useElevenLabs = false; // Ensure this is false
        initialSayVoice = 'alice';
    } else {
        useElevenLabs = false; // Default if no specific voice ID or no API key
        initialSayVoice = 'alice';
    }

    // Construct final processResponseUrl
    ...
    // --- TwiML Generation using corrected VoiceResponse class ---
    ...
    response.gather(
      { // Attributes for <Gather>
        input: 'speech dtmf',
        action: encodedProcessResponseUrl, 
        method: 'POST',
        timeout: 10, 
        speechTimeout: 'auto', 
        language: 'en-US',
      ...
      (gatherNode) => { // Callback to nest verbs inside <Gather>
        // The voice for the gather prompt should match the initial greeting voice for consistency
        gatherNode.say({ voice: initialSayVoice }, gatherPromptToSay);
        console.log(`[${requestTimestamp}] (${functionVersion}) Nested <Say voice="${initialSayVoice}"> in Gather: "${gatherPromptToSay}"`);
      }
    );
    console.log(`[${requestTimestamp}] (${functionVersion}) Gather block defined. Action: ${encodedProcessResponseUrl}`);

    // Fallback verbs *after* the <Gather> block (these are siblings to <Gather>)
    const fallbackMessage = "I'm sorry, I didn't catch that. We may need to try again later.";
    response.say({ voice: initialSayVoice }, fallbackMessage); // Use consistent voice
    console.log(`[${requestTimestamp}] (${functionVersion}) Fallback <Say voice="${initialSayVoice}">: "${fallbackMessage}"`);
    response.hangup();
    // --- End of TwiML Generation ---

    const twimlString = response.toString();
    console.log(`[${requestTimestamp}] (${functionVersion}) TwiML Being Sent (Full from Corrected Helper):`);
...
async function handleStatusCallback(req: Request, supabaseAdmin: any, requestTimestamp: string, rawBody?: string): Promise<Response> {
  try {
    ...
    let formData;
    if (rawBody && rawBody.length > 0) { // Check if rawBody has content
        ...
        // This might fail if the body was already consumed by validateTwilioRequest if it wasn't passed rawBody
        // or if the request truly had no body (e.g. some GET status callbacks if Twilio ever sends them)
        try {
            ...
            // If form data is essential and missing, might need to return an error or handle differently
            // For now, proceed cautiously, as some status updates might not have bodies or all params
            formData = new URLSearchParams(); // Empty params
        }
    }
